...
class Plantpassport:
    ...
    def load_images(self):
        self.EUflag = wand.image.Image(filename = 'euflag_black_white_contrast_60.png')
    ...
    def make_label(self, species_string, cultivar_string, nursery_string, plantID_string, country_string, isOrganic, species_nl="", label1="", label2="", label3="", label4 =""):
            # 
            #
            #
            self.load_images()
            ...
            # Assemble the image
            ppimage.composite( self.EUflag, left = 200, top = 5 )
            ppimage.composite( self.Logo, left = 1700, top = 5 )
            ...
            with wand.drawing.Drawing() as drw:
                ...
                ppimage.annotate("Plant Passport", drw, left = 320, baseline = lineheight)
                #drw.draw(ABCD)
                drw.font_weight = 400
            ...
            with wand.drawing.Drawing() as drw:
                ...
                #drw.text("Plant Passport", left=0, baseline=40)
                #drw.text(x = 0, y = lineheight, body = "Plant Passport" )
                ppimage.annotate( "Tamme Kastanje " + cultivar_string, drw, left = 1000, baseline = int(lineheight*1.5))
                #drw.draw(ABCD)
                drw.font_weight = 400
                drw.font_family = '-*-helvetica-*-r-*-*-25-*-*-*-*-138-*-*'
                drw.font_size = 25
                drw.text(x = 0, y = lineheight*5+20, body=  "Volle zon/halfschaduw")
                drw.text(x = 0, y = lineheight*6+20, body=  "Hoogte: 5-7 m")
                drw.text(x = 0, y = lineheight*7+20, body=  "Tip: Scan QR-code voor meer informatie")
                drw.text(x = textwidth, y = lineheight*5+20, body=  "Oogsttijd: oktober")
                drw.text(x = textwidth, y = lineheight*6+20, body=  "Noten vallen op de grond")
                #ABCD.text("    " + cultivar_string, drw, left=0, baseline=120)
                #ABCD.text("B. " + nursery_string, drw, left=0, baseline=160)
                #ABCD.text("C. " + plantID_string, drw, left=0, baseline=200)
                #ABCD.text("D. " + country_string, drw, left=0, baseline=240)
            ...
            # scale InfoText-image to perfectly fit between the EU flag and QR code
            InfoText.trim( )
...
if __name__ == "__main__":
    p = Plantpassport()
    p.make_label("Castanea sativa","'Marron de Lyon'","NL-150390416",'18-012345','NL',isOrganic = False)
